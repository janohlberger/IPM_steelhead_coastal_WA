...
##=================================================================##
covar_all<-read.csv(here("data","IPM_covar_dat_all.csv"))
covar_dat<-read.csv(here("data","IPM_covar_dat_selected.csv"))

##----------------------------------------------------## correlations
summary(correlation(covar_all%>%select(SST,pinks,NPGO,av_CFS_min)))
summary(correlation(covar_all%>%select(SST_cst_2,SST_2)))
summary(correlation(covar_dat%>%select(pinks_4,SST_4,NPGO_2))) #recs 
summary(correlation(covar_dat%>%select(pinks,SST,av_CFS_min))) #kelts

##========================================================## IPM fits
##----------------------------------------------## without covariates
IPM_fit_without_covars<-readRDS(here("output","IPM_fit_without_covars.rds"))
##-------------------------------------------------## with covariates
IPM_fit_with_covars<-readRDS(here("output","IPM_fit_with_covars.rds"))

##=======================================================## fish data
##----------------------------------------------## without covariates
fish_dat_without_covars<-read.csv(here("output","IPM_fish_dat_without_covars.csv"))
dat_years_without_covars<-sort(unique(fish_dat_without_covars$year))
##-------------------------------------------------## with covariates
fish_dat_with_covars<-read.csv(here("output","IPM_fish_dat_with_covars.csv"))
...
ggsave(here("output","IPM-sthd-spawner-recruit-kelt.pdf"),pp,width=1+2*nP,height=7.5)
...
save_plot(here("output","IPM-sthd-covariate-effects-recruitment-and-kelt-survival.pdf"),fig,ncol=3,nrow=2,base_height=4.5,base_width=4.5)
...
ggsave(here("output","IPM-sthd-anomalies-vs-covariates.pdf"),p,width=8.5,height=4)
