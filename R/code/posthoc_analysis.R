...
##=================================================================##
IPM_fit_no_covars<-readRDS(here("output","IPM_fit_without_covars.rds"))
...
write.csv(aic_table_recruits,here("output","AIC_table_recruits.csv"),row.names=FALSE)
...
write.csv(relimp_df,here("output","percent_recruit_var_expl_posthoc.csv"),row.names=FALSE) ## % of response variance explained

##=================================================## partial effects
pdf(here("output","IPM-sthd-recruitment-anamaly-covariates-effects.pdf"), width=4*nterms,height=4)
...
write.csv(aic_table_kelts,here("output","AIC_table_kelts.csv"),row.names=FALSE)
...
write.csv(relimp_df,here("output","percent_keltsurv_var_expl_posthoc.csv"),row.names=FALSE) ## % of response variance explained

##=================================================## partial effects
pdf(here("output","IPM-sthd-kelt-survival-anomaly-covariate-effects.pdf") ,width=4*nterms,height=4)
layout(matrix(c(1:length(mod_terms)),nrow=1,byrow=T))
par(mar=c(4,4,1,1),mgp=c(2.5,0.5,0),tcl=-0.3,cex.lab=1.5,cex.axis=1.2)
xlabels<-data.frame(term=mod_terms) %>%
   mutate(name=case_when(
...
write.csv(cov_dat_sel,here("data","IPM_covar_dat_selected.csv"),row.names=F)
